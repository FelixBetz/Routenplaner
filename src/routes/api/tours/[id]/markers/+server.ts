...
export const GET: RequestHandler = ({ params, locals }) => {
    return json(getAllMarkers(resolveTourId(params, locals)));
};
...
export const POST: RequestHandler = async ({ params, locals, request }) => {
    ...
    return json(insertMarker({ tour_id: tourId, name: name.trim(), orig_lat, orig_lon }), { status: 201 });
};
